...
# Goals and presets are siblings and live in the same place. The calendar's
# header carries one link out, not a growing list of features.
ok("/account/goals" in settings, "settings is the way in to goals")
ok("/account/presets" in settings, "and to presets")
history_page = b.get("/account").get_data(as_text=True)
eq("/account/goals" in history_page, False,
   "the calendar header does not carry a goals link")
ok("/account/settings" in history_page, "only a way through to settings")
ok("goal and preset" in settings,
   "and deleting an account says what it really removes")
...
ok("manage presets" in page, "and presets")
